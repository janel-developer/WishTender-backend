 ...
 * @param {Object} decimalMultiplierUsToPres decimal multiplier
 *
...
function Fees(
  ...
  this.accountFeeDue = accountFeeDue
    ? roundToInt(usToPresRate * decimalMultiplierUsToPres * 200)
    : 0;
  ...
  this.charge = roundToInt(
    (giftPriceTotal +
      this.accountFeeDue +
      this.appFee +
      usToPresRate * decimalMultiplierUsToPres * 55) /
  ...
  this.stripeTotalFee = roundToInt(
    ...
    usToPresRate * decimalMultiplierUsToPres * 55 + // 55 = $0.30 (stripe fee) cents +$0.25 (connected accountfee)
      this.accountFeeDue
  );
  this.stripeFee = roundToInt(this.charge * 0.029 + usToPresRate * decimalMultiplierUsToPres * 30);
  this.stripeConnectedFee = roundToInt(
    this.charge * 0.0025 + usToPresRate * decimalMultiplierUsToPres * 25
  );
